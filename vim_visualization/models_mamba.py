...
def sort_by_policy_mixer(hidden_states, policy, mixer, inference_params):
    ...
    sorted_indices_expanded = sorted_indices.unsqueeze(-1).expand(B, N, C)
    sorted_masked_hidden_states = torch.gather(masked_hidden_states, dim=1, index=sorted_indices_expanded)
    del sorted_indices_expanded

    hidden_states = mixer(sorted_masked_hidden_states, inference_params=inference_params)
    
    inverse_indices = torch.argsort(sorted_indices, dim=1)
    inverse_indices_expanded = inverse_indices.unsqueeze(-1).expand(B, N, C)
    hidden_states = torch.gather(hidden_states, dim=1, index=inverse_indices_expanded)
    del inverse_indices_expanded
        ...
        if score_predictor is not None:
            ...
            if self.training:
                ...
                if token_merge_module is not None:
                    ...
                    spatial_hidden_states = token_merge_module(
                        batch_index_select(spatial_hidden_states, keep_policy), batch_index_select(spatial_hidden_states, drop_policy), None, None, None)
                    hidden_states = torch.cat([hidden_states[:, :1, :], spatial_hidden_states], dim=1)

                    spatial_residual = residual[:, 1:]
                    spatial_residual = token_merge_module(
                        batch_index_select(spatial_residual, keep_policy), batch_index_select(spatial_residual, drop_policy), None, None, None)
                    residual = torch.cat([residual[:, :1, :], spatial_residual], dim=1)

                else:
                    hidden_states = batch_index_select(hidden_states, now_policy)
                    residual = batch_index_select(residual, now_policy)
                    #pos_embed = batch_index_select(pos_embed, now_policy)

                prev_decision = batch_index_select(prev_decision, keep_policy)

                hidden_states = self.mixer(hidden_states, inference_params=inference_params)
         
                return hidden_states, residual, prev_decision, keep_policy
...
class VisionMambaPrunning(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
            ...
            for i, layer in enumerate(self.layers):
                ...
                if i in self.pruning_loc:
                    ...
                    if self.training:
                        ...
                        hidden_states, residual, prev_decision, keep_policy = layer(
                            hidden_states, residual, inference_params=inference_params, pos_embed=None, policy=None, score_predictor = self.score_predictor, p_count=p_count,
                            prev_decision = prev_decision, early_hidden_states = early_hidden_states, token_merge_module=None, num_keep_node=num_keep_node
                        )
                        
                        decisions.append(keep_policy)
                    p_count += 1

                else:
                    if self.training:
                        hidden_states, residual = layer(
                            hidden_states, residual, inference_params=inference_params, policy=policy
                        )
                    else:
                        hidden_states, residual = layer(
        ...
        return hidden_states,prev_decision,out_pred_prob, last_pruned_features, pruning_loss, decisions
    ...
    def forward(self, x, return_features=True, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        imgs = x
        x, prev_decision, out_pred_prob,last_pruned_features, pruning_loss, decisions = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
       
        mse_loss = torch.tensor(0, device=x.device)
        if self.training:
            pred = self.decoder(x, prev_decision)
            mse_loss = self.decoder.forward_loss(imgs, pred, prev_decision)

        else:
            tokens_index = torch.arange(196).repeat(1,1).to(x.device)
            for i in range(3):
                print(decisions[i].size())
                tokens_index = batch_index_select(tokens_index, decisions[i])
           
            mask = torch.zeros(1, 196, 1,device=x.device)
            mask = mask.scatter_(1, tokens_index.unsqueeze(-1), 1)
           
            B,_,C = x.shape
            tokens_index_expand = tokens_index.unsqueeze(-1).expand(-1,-1,C)
            x_original = torch.zeros(B, 196, C, device=x.device)
            x_original = x_original.scatter_(1, tokens_index_expand, x[:,1:,])
            x = torch.cat([x[:,:1,:], x_original], dim = 1)
        ...
        if self.training:
            if self.distill:
                return x, features, prev_decision.detach(), out_pred_prob, pruning_loss, mse_loss
            else:
...
@register_model
def vimpruning_base_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_clstok_div2(pretrained=False, **kwargs):
    keep_rate = kwargs['keep_rate'] 
    model = VisionMambaPrunning(
        patch_size=16, embed_dim=768, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_divide_out=True, use_middle_cls_token=False, 
        pruning_loc=[6, 12, 18],token_merge_module=TPSModule(temperature=1), token_ratio=keep_rate, distill=False, 
...
@register_model
def vimpruning_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_clstok_div2(pretrained=False, **kwargs):
    keep_rate = kwargs['keep_rate'] 
    model = VisionMambaPrunning(
...
@register_model
def vimpruning_small_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_clstok_div2(pretrained=False, **kwargs):
    keep_rate = kwargs['keep_rate'] 
    model = VisionMambaPrunning(
        patch_size=16, stride = 8,embed_dim=384, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_divide_out=True, use_middle_cls_token=False, 
        pruning_loc=[6, 12, 18],token_merge_module=TPSModule(temperature=1), token_ratio=keep_rate, distill=False, 
        decoder_embed_dim = 512, decoder_depth=8, decoder_num_heads=16, mlp_ratio=4., norm_layer=partial(nn.LayerNorm, eps=1e-6),
        **kwargs)
    model.default_cfg = _cfg()
...
@register_model
def vimpruning_tiny_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_clstok_div2(pretrained=False, **kwargs):
    keep_rate = kwargs['keep_rate'] 
    model = VisionMambaPrunning(
        patch_size=16, stride = 8, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_divide_out=True, use_middle_cls_token=False,
        pruning_loc=[6, 12, 18], token_merge_module=TPSModule(temperature=1),token_ratio=keep_rate, distill=False, 
        decoder_embed_dim = 512, decoder_depth=8, decoder_num_heads=16, mlp_ratio=4., norm_layer=partial(nn.LayerNorm, eps=1e-6),
        **kwargs)
    model.default_cfg = _cfg()
