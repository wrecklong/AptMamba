    ...
    def forward(
        ...
        if policy is not None:
            if not self.training:
                ...
                hidden_states_original = torch.zeros(B, 196, C, device=hidden_states.device, dtype=hidden_states.dtype)

                hidden_states = hidden_states_original.scatter_(1, tokens_index_expand, hidden_states)
                policy = torch.zeros(B, 196, 1, device=hidden_states.device, dtype=hidden_states.dtype) 
                policy = policy.scatter_(1, tokens_index.unsqueeze(-1), 1)

                if residual is not None:
                    residual_original = torch.zeros(B, 196, C, device=hidden_states.device, dtype=hidden_states.dtype)
                    residual = residual_original.scatter_(1, tokens_index_expand, hidden_states)
        ...
        if policy is not None:
            if not self.training:
                pruned_part = torch.zeros(B, 196-M, C, device=hidden_states.device, dtype=hidden_states.dtype)
