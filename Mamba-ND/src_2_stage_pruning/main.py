...
from losses import DistillationLoss, PruningLoss_dynamic
from samplers import RASampler
from augment import new_data_aug_generator
from calc_flops import cal_flops, throughput
...
def get_args_parser():
    ...
    parser.add_argument('--pruning_weight', type=float, default='0.0')
    parser.add_argument('--mse_weight', type=float, default='0.0')

    parser.add_argument('--pretrained-mae-path', type=str, default='')
...
def main(args):
    ...
    #utils.init_distributed_mode_slurm(args)
    ...
    KEEP_RATE = [SPARSE_RATIO[0], SPARSE_RATIO[0] ** 2, SPARSE_RATIO[0] ** 3,
            SPARSE_RATIO[0], SPARSE_RATIO[0] ** 2, SPARSE_RATIO[0] ** 3]
    ...
    if (args.pretrained_mae_path != ''):
        checkpoint = torch.load(args.pretrained_mae_path, map_location='cpu')
        state_dict = checkpoint['model']

        checkpoint = torch.load(args.teacher_path, map_location='cpu')
        backbone_state_dict = checkpoint['state_dict']
        backbone_state_dict = {k.replace('backbone.', '', 1): v  if 'backbone' in k else v for k, v in backbone_state_dict.items()}
        backbone_state_dict = {k.replace('fc.', '', 1): v if 'fc' in k else v for k, v in backbone_state_dict.items()}
    
        utils.load_state_dict(model, backbone_state_dict)
        #decoder_state_dict = {k: v for k, v in state_dict.items() if 'decoder' in k}
        decoder_state_dict = {f"decoder.{k}": v for k, v in state_dict.items() if 'decoder' in k or 'mask' in k}
        utils.load_state_dict(model, decoder_state_dict)
    ...
    #decoder_params = list(model_without_ddp.decoder.parameters())
    #decoder_param_names = [name for name, param in model_without_ddp.decoder.named_parameters()]
   
    #other_params = [param for name, param in model_without_ddp.named_parameters() if 'decoder' not in name]

    #parameters = [
    #   {'params': other_params, 'lr': args.lr},
    #   {'params': decoder_params, 'lr':0.00015 * args.batch_size *  utils.get_world_size()/256, 'weight_decay':0.05}
    #]
    #print('other_params_lr:', args.lr)
    #print('decoder_params_lr:', 0.00015 * args.batch_size *  utils.get_world_size()/256)
    #optimizer = torch.optim.AdamW(parameters, lr = args.lr, eps = args.opt_eps, weight_decay=args.weight_decay)
    ...
    criterion = PruningLoss_dynamic(
        criterion, clf_weight=1.0, keep_ratio=KEEP_RATE, mse_token=True, ratio_weight=args.ratio_weight, reconstruction_weight=args.mse_weight, pruning_weight=args.pruning_weight
    )
