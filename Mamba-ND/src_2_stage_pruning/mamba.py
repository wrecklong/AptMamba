...
class Block(nn.Module):
    ...
    def forward(
        self, hidden_states: Tensor, residual: Optional[Tensor] = None, inference_params=None, skip=True, policy=None, **kwargs
    ):
        ...
        if policy is not None:
            if not self.training:
                ...
                hidden_states_original = torch.zeros(B, 784, C, device=hidden_states.device, dtype=hidden_states.dtype)

                hidden_states = hidden_states_original.scatter_(1, tokens_index_expand, hidden_states)
                policy = torch.zeros(B, 784, 1, device=hidden_states.device, dtype=hidden_states.dtype) 
                policy = policy.scatter_(1, tokens_index.unsqueeze(-1), 1)

                if residual is not None:
                    residual_original = torch.zeros(B, 784, C, device=hidden_states.device, dtype=hidden_states.dtype)
                    residual = residual_original.scatter_(1, tokens_index_expand, residual)
                
                del tokens_index_expand
        ...
        if self.transpose:
            ...
            h = w = int(np.sqrt(l))
            if (policy is not None):
                policy = rearrange(policy,'n (h w) c -> n (w h) c',h=h,w=w)
        ...
        if policy is not None:
            ...
            if residual is not None:
                masked_residual = residual * policy
                residual = torch.gather(masked_residual, dim=1, index=sorted_indices_expanded)
        ...
        if policy is not None:
            if not self.training:
                pruned_part = torch.zeros(B, 784-M, C, device=hidden_states.device, dtype=hidden_states.dtype)
                hidden_states = torch.cat([hidden_states, pruned_part], dim=1)
                if residual is not None:
                    residual = torch.cat([residual, pruned_part], dim=1)
                del pruned_part
            ...
            if residual is not None:
                residual = torch.gather(residual, dim=1, index=inverse_indices_expanded)
        ...
        if not self.training:
            if policy is not None:
                hidden_states = batch_index_select(hidden_states, tokens_index)
                if residual is not None:
                    residual = batch_index_select(residual, tokens_index)
...
class Mamba2DModel_Pruning(nn.Module):
    """Vision Transformer.
    ...
    def __init__(self,
        ...
        predictor_list = [PredictorLG(self.embed_dims) for _ in range(len(pruning_loc))]
        self.score_predictor = nn.ModuleList(predictor_list)
 
        self.head = nn.Linear(self.embed_dims, num_classes) if num_classes > 0 else nn.Identity()
        
        self.decoder = MAE_Decoder(embed_dim=self.embed_dims,num_patches=num_patches//4,
                                   patch_size=patch_size*2,decoder_embed_dim=decoder_embed_dim,
    ...
    def forward_features(self, x):
        ...
        x_original = torch.zeros(B, init_n, C, device=x.device, dtype=x.dtype)
        residual_original = torch.zeros(B, init_n, C, device=x.device, dtype=x.dtype)
        ...
        if self.update_interval:
                ...
                if self.use_nd:
                    ...
                    if i in self.pruning_loc:
                        ...
                        if self.training:
                            ...
                            if (p_count < 3):
                                x_pruned = x * ((1 - hard_keep_decision) * prev_decision)
                                x_pruned_list.append(x_pruned)

                            pruning_loss = pruning_loss + early_pruning_loss(x, hard_keep_decision, current_pruned_decision)
                            x,residual = layer(x, residual, policy = prev_decision, h=h,w=w)
                            ...
                            pruned_policy = torch.argsort(score, dim=1, descending=True)[:, :num_keep_node]
                            ...
                            if (p_count < 3):
                                ...
                                if residual is not None:
                                    residual_pruned = batch_index_select(residual, pruned_policy)

                                x_original = x_original.scatter_(1, pruned_tokens_index_expand, x_pruned)

                                if residual is not None:
                                    residual_original =  residual_original.scatter_(1, pruned_tokens_index_expand, residual_pruned)
                            prev_decision = batch_index_select(prev_decision, keep_policy)
                            x = batch_index_select(x, keep_policy)

                            if residual is not None:
                                residual = batch_index_select(residual, keep_policy)

                            x,residual = layer(x, residual, policy=tokens_index, h=h,w=w)                
                        ...
                        if self.training:
                            for i in range(len(x_pruned_list)):
                                x = x + x_pruned_list[i]
                        else:
                            B,_,C = x.shape
                            tokens_index_expand = tokens_index.unsqueeze(-1).expand(-1,-1,C)
            ...
            #x = self.ln2((x * prev_decision).sum(dim=1, keepdim=True) / torch.sum(prev_decision, dim=1, keepdim=True))
            

                # if i in self.out_indices:
                #     outs.append(self._format_output(x, patch_resolution))

        return x, out_pred_prob, prev_decision, pruning_loss
    ...
    def forward(self, x):
        imgs = x
        x, out_pred_prob, prev_decision, pruning_loss = self.forward_features(x)
        ...
        if self.training:
            ...
            #     pred = self.feature_decoder(prev_features[i], out_pred_prob[i])
            #     loss += self.feature_decoder.forward_loss(pred, prev_mask[i], out_pred_prob[i], prev_features[i])
        x = self.ln2((x * prev_decision).sum(dim=1, keepdim=True) / torch.sum(prev_decision, dim=1, keepdim=True))
        #x = self.ln2(x.mean(1))
        x = x.squeeze(1)
        x = self.head(x)
...
@register_model
def MambaND_pruning_small_patch8_224(pretrained=False, **kwargs):
    keep_rate = kwargs['keep_rate'] 
    model = Mamba2DModel_Pruning(
        arch='small', img_size=224, patch_size=8, out_type='avg_featmap',  drop_path_rate=0.1, drop_rate=0.1,
        with_cls_token=False, final_norm=True,fused_add_norm=False, d_state=16, is_2d=False,use_v2=False, use_nd=False,
        constant_dim=True, downsample=(9,), force_a2=False, use_mlp=False, 
        pruning_loc=[2,5,8,11,15,19], token_ratio=keep_rate, 
...
@register_model
def MambaND_pruning_base_patch8_224(pretrained=False, **kwargs):
    keep_rate = kwargs['keep_rate'] 
    model = Mamba2DModel_Pruning(
        arch='base', img_size=224, patch_size=8, out_type='avg_featmap',  drop_path_rate=0.1, drop_rate=0.1,
        with_cls_token=False, final_norm=True,fused_add_norm=False, d_state=16, is_2d=False,use_v2=False, use_nd=False,
        constant_dim=True, downsample=(9,), force_a2=False, use_mlp=False, 
        pruning_loc=[10, 14, 18], token_ratio=keep_rate, 
        decoder_embed_dim = 512, decoder_depth=8, decoder_num_heads=16, mlp_ratio=4., norm_layer=partial(nn.LayerNorm, eps=1e-6),
    )
    #model.default_cfg = _cfg()
    if pretrained:
