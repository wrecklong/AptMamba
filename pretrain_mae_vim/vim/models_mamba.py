...
class VisionMamba_MAE(nn.Module):
    def __init__(self, 
                 img_size=224, 
                 patch_size=16, 
                 stride=16,
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size**2 * channels, bias=True) # decoder to patch
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False, mask_ratio = 0.75):
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False, mask_ratio=0.75):
        imgs = x
        x, mask, ids_restore = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank, mask_ratio=mask_ratio)
...
@register_model
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_clstok_div2_mae(pretrained=False, **kwargs):
    model = VisionMamba_MAE(
        ...
        decoder_embed_dim = 512, decoder_depth=8, decoder_num_heads=16, mlp_ratio=4., norm_layer=partial(nn.LayerNorm, eps=1e-6),**kwargs)
...
@register_model
def vim_tiny_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_clstok_div2_mae(pretrained=False, **kwargs):
    model = VisionMamba_MAE(
