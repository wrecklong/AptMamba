...
def sort_by_policy_mixer(hidden_states, policy, mixer, inference_params):
    ...
    hidden_states = mixer(masked_hidden_states, inference_params=inference_params)
...
class VisionMambaPrunning(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        #tokens_index = torch.arange(init_n).repeat(B,1).to(hidden_states.device) 
        if not self.if_bidirectional:
            ...
            for i, layer in enumerate(self.layers):
                ...
                if i in self.pruning_loc:
                    ...
                    N = N - 1
                    tokens_index = torch.arange(N).repeat(B,1).to(hidden_states.device)
                    rand_vals = torch.rand(B, N).to(hidden_states.device)
                    
                    num_select = int(N * 0.7)
                    _, indices = torch.topk(rand_vals, num_select, dim=1)

                    
                    mask = torch.zeros(B, N, dtype=torch.bool,device=hidden_states.device)
                    mask.scatter_(1, indices, True)

                    tokens_index = tokens_index[mask].view(B, -1)
                    
                    cls_token = hidden_states[:,:1,:]
                    spatial_hidden_states = hidden_states[:,1:,:]
                    spatial_hidden_states = batch_index_select(spatial_hidden_states, tokens_index)
                    hidden_states = torch.cat([cls_token, spatial_hidden_states],dim=1) 

                    cls_token_r = residual[:,:1,:]   
                    spatial_residual = residual[:,1:,: ] 
                    spatial_residual = batch_index_select(spatial_residual, tokens_index)


                    residual = torch.cat([cls_token_r, spatial_residual],dim=1)
        ...
        return hidden_states,prev_decision, out_pred_prob
    ...
    def forward(self, x, return_features=True, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        imgs = x
        x, prev_decision, out_pred_prob = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
       
        mse_loss = torch.tensor(0, device=x.device)
        pruning_loss = torch.tensor(0, device=x.device)
        
        # if self.training and self.stride == 16:
        ...
        if self.training:
            if self.distill:
                ...
                return x, out_pred_prob, pruning_loss, mse_loss
