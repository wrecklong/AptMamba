...
from losses import DistillationLoss, DistillDiffPruningLoss_dynamic
...
def main(args):
    ...
    #score_predictor_params = list(model_without_ddp.score_predictor.parameters())
    #decoder_param_names = [name for name, param in model_without_ddp.decoder.named_parameters()]
   
    #other_params = [param for name, param in model_without_ddp.named_parameters() if 'predictor' not in name]

    #parameters = [
    #    {'params': other_params, 'lr': args.lr},
    #    {'params': score_predictor_params, 'lr': args.lr * 5}
    #]
    #print('score_predictor_params',args.lr*5)
    #print('other_params',args.lr)
    ...
    criterion = DistillDiffPruningLoss_dynamic(
        teacher_model, criterion, clf_weight=1.0, keep_ratio=KEEP_RATE, mse_token=True, ratio_weight=args.ratio_weight,distill_weight=0.0,reconstruction_weight =0.0, pruning_weight=0.0
    )
