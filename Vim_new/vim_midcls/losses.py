...
class DistillDiffPruningLoss_dynamic(torch.nn.Module):
    ...
    taking a teacher model prediction and using it as additional supervision.
    """
    def __init__(self, teacher_model, base_criterion: torch.nn.Module, ratio_weight=2.0, distill_weight=0.5, reconstruction_weight = 0.5, pruning_weight = 0.1, dynamic=False, pruning_loc=[6,12,18], keep_ratio=[0.75, 0.5, 0.25], clf_weight=0, mse_token=False, print_mode=True):
        super().__init__()
        self.teacher_model = teacher_model
        self.base_criterion = base_criterion
    ...
    def forward(self, inputs, outputs, labels):
        ...
        pred, token_pred, mask, out_pred_score, pruning_loss = outputs
        ...
        loss = self.clf_weight * cls_loss + self.ratio_weight * pred_loss / len(self.pruning_loc) + self.pruning_weight * pruning_loss
        ...
        if self.print_mode:
            ...
            if self.count == 100:
                ...
                print('loss info: cls_loss=%.4f, ratio_loss=%.4f, pruning_loss=%.4f' % (self.cls_loss / 100, self.ratio_loss / 100, self.pruning_loss/100))
                self.count = 0
                self.cls_loss = 0
                self.ratio_loss = 0
                self.cls_distill_loss = 0
